...
- **deploy/** – Deployment and ops: `install.sh`, `setup-instance.sh`, `init-ssl.sh`, `setup-s3-assets.sh`, `backup-database.sh`, `restore-database.sh`, `deploy-production.sh`, `setup-firewall.sh`, `setup-fail2ban.sh`, `uninstall.sh`, etc. Run from **project root**. See [docs/setup.md](../docs/setup.md).
