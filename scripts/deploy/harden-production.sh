...
if [ -f "$BACKUP_SCRIPT" ]; then
    ...
    if ! crontab -l 2>/dev/null | grep -q "backup-database.sh"; then
        (crontab -l 2>/dev/null; echo "0 2 * * * $BACKUP_SCRIPT >> /var/log/maestro-backup.log 2>&1") | crontab -
...
cat > /etc/sysctl.d/99-maestro-security.conf <<'EOF'
...
sysctl -p /etc/sysctl.d/99-maestro-security.conf
...
echo "  - Monitor: docker logs -f maestro-app"
