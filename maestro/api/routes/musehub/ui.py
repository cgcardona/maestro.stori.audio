    ...
    script = f"""
      ...
      function renderGrooveChart(entries, width, height) {{
        ...
        for (let i = 0; i <= 4; i++) {{
          ...
          grid += '<line x1="' + pad.left + '" y1="' + y + '" x2="' + (width - pad.right) + '" y2="' + y + '" stroke="#21262d" stroke-width="1"/>';
          grid += '<text x="' + (pad.left - 4) + '" y="' + (y + 4) + '" font-size="9" fill="#8b949e" text-anchor="end">' + val + '</text>';
        }}
        ...
        entries.forEach((e, i) => {{
          ...
          bars += '<rect x="' + x + '" y="' + y + '" width="' + barW + '" height="' + barH + '" fill="' + color + '" opacity="0.85" rx="1"><title>' + escHtml(e.commit) + ' score: ' + e.grooveScore.toFixed(4) + ' (' + e.status + ')</title></rect>';
          if (entries.length <= 14 || i % 2 === 0) {{
            bars += '<text x="' + (x + barW / 2) + '" y="' + (height - 6) + '" font-size="9" fill="#8b949e" text-anchor="middle" transform="rotate(-30 ' + (x + barW / 2) + ' ' + (height - 6) + ')">' + escHtml(e.commit.substring(0,6)) + '</text>';
          }}
        }});

        const axisLabel = '<text x="10" y="' + (height / 2) + '" font-size="10" fill="#8b949e" transform="rotate(-90 10 ' + (height / 2) + ')" text-anchor="middle">groove score (beats)</text>';

        return '<svg width="100%" viewBox="0 0 ' + width + ' ' + height + '" style="display:block;overflow:visible" preserveAspectRatio="xMidYMid meet">' + grid + bars + axisLabel + '</svg>';
      }}
      ...
      function statusBadge(status) {{
        ...
        return '<span style="display:inline-block;padding:1px 8px;border-radius:10px;font-size:11px;font-weight:700;background:' + bg + ';color:#0d1117">' + escHtml(status) + '</span>';
      }}
      ...
      async function load(threshold, limit) {{
        ...
        try {{
          ...
          const tableRows = entries.length === 0
            ...
            : entries.map(e => '<tr><td style="font-family:monospace;font-size:13px"><a href="' + base + '/commits/' + e.commit + '">' + escHtml(e.commit) + '</a></td><td>' + statusBadge(e.status) + '</td><td style="font-size:13px">' + e.grooveScore.toFixed(4) + '</td><td style="font-size:13px">' + (e.driftDelta > 0 ? '+' : '') + e.driftDelta.toFixed(4) + '</td><td style="font-size:13px;color:#8b949e">' + escHtml(e.track) + '</td><td style="font-size:13px;color:#8b949e">' + e.midiFiles + '</td></tr>').join('');
          ...
          document.getElementById('content').innerHTML = `
            ...
            <div class="card">
              ...
              <div style="background:#0d1117;border:1px solid #21262d;border-radius:6px;padding:12px;overflow-x:auto;margin-bottom:4px">
                ${{chartHtml}}
              </div>
