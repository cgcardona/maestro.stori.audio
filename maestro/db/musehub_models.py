...
class MusehubCollaborator(Base):
    """An explicit collaborator grant on a Muse Hub repo.

    The ``permission`` field controls what the grantee may do:
    - ``write``: push commits and branches
    - ``admin``: all write permissions plus settings and collaborator management

    ``accepted_at`` is null until the invitee accepts; the grant is not active
    until accepted.  ``invited_by`` tracks the inviter for audit purposes.
    """
