...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/musehub/ui/{repo_id}/analysis/{ref}` | HTML dashboard with 10 dimension summary cards (no auth required) |
...
- A clickable link to the per-dimension analysis page at `/{repo_id}/analysis/{ref}/{dim_id}`

**Branch/tag selector:** A `<select>` populated by `GET /api/v1/musehub/repos/{repo_id}/branches`. Changing the selected branch navigates to `/{repo_id}/analysis/{branch}`.
...
**Auth model:** The HTML page at `/musehub/ui/.../analysis/{ref}` is a JS shell — no server-side auth required. The embedded JavaScript fetches the API with a JWT from `localStorage`.
...
| UI tests | `tests/test_musehub_ui.py` — `test_analysis_dashboard_*` |
| API tests | `tests/test_musehub_analysis.py` — `test_analysis_aggregate_endpoint` |
