...
Pydantic `BaseModel` — A single project entry in `pipeline-config.json`. Added by AC-601 to support multi-repo generalisation. Each project maps to a distinct GitHub repository and local workspace. `AgentCeptionSettings._apply_active_project()` reads the active entry at initialisation to override path defaults.
...
| `active_labels_order` | `list[str]` | Ordered list of `agentception/*` phase labels for this project |
...
Pydantic `BaseModel` — Request body for `POST /api/config/switch-project`. Added by AC-601.
...
Sets `active_project` in `pipeline-config.json` and returns the updated config. Raises `ValueError` when `project_name` does not match any entry in `PipelineConfig.projects`. Added by AC-601.
