...
| **Music** | `STORI_ORPHEUS_BASE_URL` (default `http://localhost:10002`), `STORI_HF_API_KEY`, `STORI_ORPHEUS_MAX_CONCURRENT` (default `4` — max parallel GPU inference calls), `STORI_ORPHEUS_TIMEOUT` (default `360`s), `STORI_ORPHEUS_CB_THRESHOLD` (default `3` — consecutive failures before circuit breaker trips), `STORI_ORPHEUS_CB_COOLDOWN` (default `60`s — probe interval after trip). **Docker:** `docker-compose.yml` overrides to `http://orpheus:10002` so the maestro container can reach Orpheus. See **HuggingFace token (Orpheus)** below. |
