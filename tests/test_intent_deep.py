...
class TestAffirmativeNegative:
    ...
    def test_long_phrase_not_affirmative(self):
        from app.core.intent import _is_affirmative
...
class TestIntentWithLLM:
    ...
    @pytest.mark.anyio
    async def test_pattern_hit_no_llm_needed(self):
        result = await get_intent_result_with_llm("play")
        assert result.intent == Intent.PLAY

    @pytest.mark.anyio
    async def test_unknown_triggers_llm(self):
        mock_llm = AsyncMock()
    ...
    @pytest.mark.anyio
    async def test_affirmative_with_context(self):
        history = [
    ...
    @pytest.mark.anyio
    async def test_affirmative_without_context(self):
        result = await get_intent_result_with_llm("yes")
        assert isinstance(result, IntentResult)

    @pytest.mark.anyio
    async def test_no_llm_fallback_when_none(self):
...
class TestHelpers:
    ...
    def test_extract_quoted_double(self):
    ...
    def test_extract_quoted_none(self):
        from app.core.intent import _extract_quoted
