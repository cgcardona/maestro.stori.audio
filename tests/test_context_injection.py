...
class TestStructuredPromptContextStandalone:
    ...
    def test_closing_sentinel_present(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
        assert "═════════════════════════════════════" in ctx

    def test_do_not_reinfer_instruction(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
        assert "Do not re-infer" in ctx

    def test_mode_always_emitted(self):
    ...
    def test_section_absent_when_not_set(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
    ...
    def test_maestro_dims_in_context(self):
        parsed = _parse(
            "STORI PROMPT\nMode: compose\n"
            "Harmony:\n  progression: ii-V-I\n"
            "Request: go\n"
    ...
    def test_no_maestro_dims_when_none(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
...
class TestSequentialContextStandalone:
    ...
    def test_absolute_contains_beat(self):
    ...
    def test_section_name_appears(self):
    ...
    def test_pickup_mentions_lead_in(self):
...
class TestCombinedContextNoDuplicates:
    ...
    def test_arrangement_position_appears_exactly_once(self):
        ctx = self._combined(
    ...
    def test_stori_structured_sentinel_exactly_once(self):
        ctx = self._combined(
            "STORI PROMPT\nMode: compose\nPosition: at 32\nRequest: go"
        )
        assert ctx.count("═══ STORI STRUCTURED INPUT ═══") == 1

    def test_structured_context_before_sequential_context(self):
        ctx = self._combined(
            "STORI PROMPT\nMode: compose\nSection: verse\nPosition: after intro\nRequest: go",
            {"tracks": [{"name": "intro", "regions": [
                {"name": "intro", "startBeat": 0, "durationBeats": 16}
    ...
    def test_mode_field_not_duplicated(self):
        ctx = self._combined(
            "STORI PROMPT\nMode: compose\nPosition: last\nRequest: go"
        )
        assert ctx.count("Mode: compose") == 1

    def test_no_position_no_arrangement_block(self):
        ctx = self._combined("STORI PROMPT\nMode: compose\nRequest: go")
        assert "ARRANGEMENT POSITION" not in ctx

    def test_all_maestro_dims_and_position_coexist(self):
        ctx = self._combined(
            "STORI PROMPT\nMode: compose\nSection: verse\nPosition: after intro\n"
            "Harmony:\n  progression: ii-V-I\n"
            "Request: go",
...
class TestFullSystemPromptAssembly:
    ...
    def test_composing_system_has_base(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
    ...
    def test_composing_system_has_structured_block(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nStyle: jazz\nRequest: go")
        sys = _full_composing_system(parsed)
        assert "STORI STRUCTURED INPUT" in sys
        assert "jazz" in sys

    def test_composing_system_with_position_has_arrangement_block(self):
        parsed = _parse(
            "STORI PROMPT\nMode: compose\nSection: verse\nPosition: after intro\nRequest: go"
        )
        project = {"tracks": [
    ...
    def test_editing_system_has_structured_block(self):
        parsed = _parse("STORI PROMPT\nMode: edit\nTarget: track:Drums\nRequest: go")
    ...
    def test_do_not_reinfer_present(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nStyle: jazz\nRequest: go")
        sys = _full_composing_system(parsed)
...
class TestAbsentFieldsNotInjected:
    ...
    def test_style_absent_when_not_set(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
        assert "Style:" not in ctx

    def test_key_absent_when_not_set(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
        assert "Key:" not in ctx

    def test_tempo_absent_when_not_set(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
        assert "Tempo:" not in ctx

    def test_roles_absent_when_not_set(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
        assert "Roles:" not in ctx

    def test_target_absent_when_not_set(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
    ...
    def test_constraints_absent_when_not_set(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
        assert "Constraints:" not in ctx

    def test_vibes_absent_when_not_set(self):
        parsed = _parse("STORI PROMPT\nMode: compose\nRequest: go")
        ctx = structured_prompt_context(parsed)
