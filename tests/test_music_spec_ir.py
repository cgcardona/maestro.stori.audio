...
class TestBuildFullMusicSpec:
    ...
    def test_section_map_long(self):
...
class TestDataclasses:
    ...
    def test_global_spec_defaults(self):
        gs = GlobalSpec()
        assert gs.tempo == 120
        assert gs.bars == 16
        assert gs.time_signature == (4, 4)

    def test_section_map_entry(self):
    ...
    def test_drum_constraints(self):
        dc = DrumConstraints(fill_bars=[3, 7])
        assert dc.fill_bars == [3, 7]

    def test_chord_schedule_entry(self):
