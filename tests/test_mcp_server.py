...
def mcp_server():
    with patch("app.config.get_settings") as mock_settings:
        mock_settings.return_value = MagicMock(app_version="1.0.0-test")
...
class TestStoriMCPServer:
    ...
    def test_register_and_unregister_daw(self, mcp_server):
        cb = AsyncMock()
    ...
    def test_receive_response_nonexistent_connection(self, mcp_server):
        mcp_server.receive_tool_response("ghost", "req-1", {})  # Should not raise

    def test_receive_response_nonexistent_request(self, mcp_server):
        cb = AsyncMock()
        mcp_server.register_daw("daw-1", cb)
        mcp_server.receive_tool_response("daw-1", "no-req", {})  # Should not raise

    def test_multiple_daw_connections(self, mcp_server):
...
class TestCallTool:
    ...
    @pytest.mark.anyio
    async def test_valid_daw_tool_no_connection(self, mcp_server):
    ...
    @pytest.mark.anyio
    async def test_invalid_params(self, mcp_server):
