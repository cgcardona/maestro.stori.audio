...
class TestNFCNormalisation:
    ...
    def test_precomposed_e_preserved(self):
        """Precomposed é (U+00E9) is kept as-is."""
        result = normalise_user_input("café")
        assert "é" in result

    def test_decomposed_e_composed_to_nfc(self):
...
class TestInvisibleCharStripping:
    ...
    def test_music_chars_preserved(self):
...
class TestBlankLineCollapsing:
    ...
    def test_five_blank_lines_collapsed(self):
        result = normalise_user_input("a\n\n\n\n\n\nb")
        assert "\n\n\n" not in result
    ...
    def test_padding_attack_collapsed(self):
...
class TestTrailingWhitespaceStripping:
    ...
    def test_trailing_spaces_on_each_line(self):
...
class TestStructuredPromptRoundTrip:
    ...
    def test_prompt_with_crlf_line_endings_parses(self):
        from app.core.prompt_parser import parse_prompt
        prompt = "STORI PROMPT\r\nMode: compose\r\nRequest: go\r\n"
        result = parse_prompt(normalise_user_input(prompt))
        assert result is not None

    def test_prompt_with_invisible_chars_stripped_then_parses(self):
        from app.core.prompt_parser import parse_prompt
        prompt = "STORI\u200b PROMPT\nMode: compose\nRequest: go"
        result = parse_prompt(normalise_user_input(prompt))
        assert result is not None

    def test_prompt_with_control_chars_stripped_then_parses(self):
        from app.core.prompt_parser import parse_prompt
    ...
    def test_yaml_block_scalar_indentation_preserved(self):
        from app.core.prompt_parser import parse_prompt
        prompt = (
            "STORI PROMPT\n"
            "Mode: compose\n"
...
class TestSanitizeSecurity:
    ...
    def test_mixed_injection_attempt_cleaned(self):
