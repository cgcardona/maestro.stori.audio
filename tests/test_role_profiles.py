...
class TestRoleProfileLoading:
    ...
    def test_expected_roles_loaded(self):
        assert set(ROLE_PROFILES.keys()) == {"lead", "bass", "chords", "pads", "drums"}

    def test_track_counts_plausible(self):
    ...
    def test_lead_has_most_tracks(self):
...
class TestRoleProfileFields:
    ...
    def test_syncopation_within_zero_one(self):
        for name, profile in ROLE_PROFILES.items():
...
class TestFuzzyLookup:
    ...
    def test_alias_resolution(self, alias: str, expected_role: str):
...
class TestPromptBlock:
    ...
    def test_prompt_block_contains_role_name(self):
        lead = ROLE_PROFILES["lead"]
        block = lead.prompt_block()
        assert "LEAD" in block
        assert "MUSICAL DNA" in block

    def test_prompt_block_contains_key_sections(self):
    ...
    def test_prompt_block_contains_track_count(self):
        pads = ROLE_PROFILES["pads"]
        block = pads.prompt_block()
        assert str(pads.track_count) in block.replace(",", "")

    def test_prompt_block_polyphony_description(self):
...
class TestOrpheusConditioning:
    ...
    def test_density_hint_valid(self):
    ...
    def test_drums_low_complexity(self):
        drums = ROLE_PROFILES["drums"]
        assert drums.orpheus_complexity < 0.2

    def test_chords_moderate_complexity(self):
        chords = ROLE_PROFILES["chords"]
